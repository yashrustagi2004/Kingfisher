  ...
  const routes = {
    ...
    'trusted-domains': async () => {
      ...
      try {
        ...
        contentBody.innerHTML = `<ul>${domains.map((domain) => `<li>${domain}</li>`).join('')}</ul>`;
      } catch (error) {
    ...
    'malicious-domains': async () => {
      ...
      contentBody.innerHTML = '<p>Loading...</p>';
      try {
        const response = await fetch('http://localhost:4000/settings/malicious-domains');
        const data = await response.json();
        const malicious = data.malicious || [];
        contentBody.innerHTML = `<ul>${malicious.map((domain) => `<li>${domain}</li>`).join('')}</ul>`;
      } catch (error) {
    ...
    tips: async () => {
      ...
      try {
        ...
        contentBody.innerHTML = `<ul>${tips.map((tip) => `<li>${tip}</li>`).join('')}</ul>`;
      } catch (error) {
